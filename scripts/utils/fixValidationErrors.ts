...
function getCSVUrlMap(): { vaultUrlMap: Map<string, string>; protocolUrlMap: Map<string, string> } {
  ...
  try {
    ...
    for (const line of lines) {
      ...
      if (parts.length >= 13) {
        const vaultAddress = parts[10]?.trim().replace(/"/g, '').toLowerCase() || '';
  ...
  try {
    ...
    for (const line of lines) {
      ...
      if (parts.length >= 13) {
        ...
        if (/^0x[a-fA-F0-9]{40}$/.test(vaultAddress) && /^0x[a-fA-F0-9]{40}$/.test(stakingTokenAddress)) {
...
function fixValidationErrors(): void {
  ...
  const mainnetData: MainnetData = JSON.parse(fs.readFileSync(mainnetPath, 'utf-8'));
  ...
  for (const vault of mainnetData.vaults) {
    ...
    if (stakingTokenMap.has(vaultAddrLower)) {
      vault.stakingTokenAddress = stakingTokenMap.get(vaultAddrLower)!;
    ...
    // Fix URL
    const url = vault.url || '';
    if (url.startsWith(PLACEHOLDER_URL_BASE) || !url || !url.startsWith('https://')) {
      // Try to get from CSV
  ...
  console.log("- Updated placeholder URLs from CSV or set to berachain.com if missing");
  console.log("- Set stakingTokenAddress for vaults from CSV");
  console.log("- Ensured all vaults and protocols have valid unique https:// URLs");
